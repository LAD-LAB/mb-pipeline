...
# scripts.dir='/hpc/group/ldavidlab/users/tjk30/mb-pipeline/code'
...
print(paste0(
  'sbatch --mail-user=',email,scripts.dir,'/demux-barcode.sh ', miniseqDir, ' ', samplesheet, ' ', demux.container
  ))
```
Paste the above command. Ie:
```{bash}
# Example
sbatch --mail-user=youremail@duke.edu /path/to/demux-barcode.sh /path/to/miniseq-dir /path/to/samplesheet /hpc/group/ldavidlab/metabarcoding.sif
```
...
```{r}
print('trnL')
print(paste0('sbatch --mail-user=',email,scripts.dir,'/count-reads.sh ',dirname(demux.dir),'/',format(Sys.Date(),'%Y%m%d'),'_trnL_results', ' ', qiime.container
))
print('12SV5')
print(paste0('sbatch --mail-user=',email,scripts.dir,'/count-reads.sh ',dirname(demux.dir),'/',format(Sys.Date(),'%Y%m%d'),'_12SV5_results', ' ', qiime.container
))
```
