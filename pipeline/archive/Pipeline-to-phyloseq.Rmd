...
# Which accessions are missing in the sql database? Manually add them below
print(taxtab.species.sep[which(is.na(taxids)),2:3])
```

To manually add a taxid for a missing accession, you need the accession and taxid; take the accession from above and look up the taxid with NCBI's Nucleotide database.

```{r}
# if (any(taxtab.species.sep$accession == "accession")) {
  # taxids[which(taxtab.species.sep$accession == "accession")] <- taxid
# }
